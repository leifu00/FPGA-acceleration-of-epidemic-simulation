...
// SystemVerilog created from infect_sweep_function_cra_slave
...
module infect_sweep_function_cra_slave (
    ...
    // start_bit(BITSELECT,191)
    assign start_bit_b = status_NO_SHIFT_REG_q[0:0];

    // start_or_start_buffered(LOGICAL,196)
    ...
    // start_buffered_and_kernel_idle(LOGICAL,193)
    assign start_buffered_and_kernel_idle_q = not_running_bit_q & buffered_start_NO_SHIFT_REG_q;

    // start_from_buffered_start_NO_SHIFT_REG(REG,194)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // readdata_valid_stage1_reg(REG,178)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // start_bit_computation(LOGICAL,192)
    assign start_bit_computation_q = not_start_q & start_bit_b;

    // status_self_update(BITJOIN,205)
    ...
    // readdata_upper_bits_mux(MUX,177)
    ...
    // readdata_valid_stage2_reg(REG,179)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // dupName_0_ctrl_profile_status_bit_x(BITSELECT,215)
    assign dupName_0_ctrl_profile_status_bit_x_b = status_NO_SHIFT_REG_q[5:5];

    // status_done_bit(BITSELECT,200)
    ...
    // dupName_7_address_ref_x(CONSTANT,257)
    assign dupName_7_address_ref_x_q = $unsigned(5'b01000);

    // dupName_7_NO_NAME_x(LOGICAL,256)
    ...
    // start_is_or_going_high(LOGICAL,195)
